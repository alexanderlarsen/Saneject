...
namespace Plugins.Saneject.Legacy.Editor.Inspectors.API
{
    public static class SanejectInspector
    {
        ...
        {
            List<PropertyData> list = new();
            CollectForType(serializedObject, null, target.GetType(), list);
            properties = list;
        }

        /// <summary>
        /// Recursively collects PropertyData for all serializable fields of the given type and its base classes, including [SerializeInterface] fields and nested serializable types.
        /// </summary>
        private static void CollectForType(
            ...
            List<PropertyData> list)
        {
            foreach (FieldInfo field in type.GetAllFields())
            {
                ...
                if (field.FieldType.IsNestedSerializable())
                {
                    ...
                    CollectForType(root, property, field.FieldType, children);
                }
