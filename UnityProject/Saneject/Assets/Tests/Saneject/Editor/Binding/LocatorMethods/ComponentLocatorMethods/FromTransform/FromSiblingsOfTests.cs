...
namespace Tests.Saneject.Editor.Binding.LocatorMethods.ComponentLocatorMethods.FromTransform
{
    public class FromSiblingsOfTests
    {
        ...
        [Test]
        public void FromSiblingsOf_TConcrete_InjectsToConcreteCollection()
        {
            // Set up scene
            TestScene scene = TestScene.Create(roots: 2, width: 3, depth: 3);
            TestScope scope = scene.Add<TestScope>("Root 1");
            ...
            {
                scene.Add<ComponentDependency>("Root 2/Child 2/Child 1"),
                scene.Add<ComponentDependency>("Root 2/Child 2/Child 2")
            };

            // Bind
            scope.BindComponents<ComponentDependency>().FromSiblingsOf(transform);
