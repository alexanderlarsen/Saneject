...
namespace Tests.Saneject.Editor.Binding.Components.Locators.FromScope
{
    public class FromScopeDescendantsTests
    {
        ...
        [Test]
        public void FromScopeDescendants_WHEN_IncludeSelfIsFalse_TConcrete_InjectsToConcreteCollection()
        {
            // Set up scene
            TestScene scene = TestScene.Create(roots: 1, width: 3, depth: 3);
            TestScope scope = scene.Add<TestScope>("Root 1");
            ...
            // Bind
            scope.BindComponents<ComponentDependency>().FromDescendants(includeSelf: false);
