...
{
    public class FromLastChildOfTests
    {
        [Test]
        ...
        public void FromLastChildOf_TInterface_InjectsToInterfaceField()
        {
            ...
            // Bind
            scope.BindComponent<IDependency>().FromLastChildOf(transform);
        ...
        [Test]
        public void FromLastChildOf_TInterfaceTConcrete_InjectsToInterfaceField()
        {
            // Set up scene
            TestScene scene = TestScene.Create(roots: 2, width: 3, depth: 3);
            scene.AddToAllTransforms<ComponentDependency>();
        ...
        [Test]
        public void FromLastChildOf_TConcrete_InjectsToConcreteCollection()
        {
            // Set up scene
            TestScene scene = TestScene.Create(roots: 2, width: 3, depth: 3);
            TestScope scope = scene.Add<TestScope>("Root 1");
            ...
            {
                scene.Add<ComponentDependency>("Root 2/Child 3"),
                scene.Add<ComponentDependency>("Root 2/Child 3")
            };

            // Bind
            scope.BindComponents<ComponentDependency>().FromLastChildOf(transform);
