...
This page explains the editor-side system that prepares injected scenes and prefabs. It is about architecture and lifecycle, not the fluent binding API. For the individual concepts, see [Scope](../core-concepts/scope.md), [Binding](../core-concepts/binding.md), [Context](../core-concepts/context.md), and [Field, property & method injection](../core-concepts/field-property-and-method-injection.md).
...
- Component bindings whose concrete type is not a `Component`
- Asset bindings whose concrete type incorrectly derives from `Component`
...
One subtle but important detail is that non-`GameObject` objects such as assets and proxy assets have `ContextType.Global`, so they are not rejected by context isolation. This is what makes asset injection and proxy placeholder injection compatible with strict context boundaries.
...
1. Inject resolved global components into each active scope's hidden global list
2. Assign field and auto-property backing field values by reflection
...
1. Clearing the current proxy swap target list on every active scope
...
4. Registering the owning component on the nearest scope if that component implements `IRuntimeProxySwapTarget`
