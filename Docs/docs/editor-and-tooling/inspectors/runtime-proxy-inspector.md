...
3. Find the new generated proxy asset to confirm the changes.
...
Matches `RuntimeProxyResolveMethod` and defines how the proxy finds or creates its target at runtime.
...
- `Transient`: Creates a new instance each time the proxy resolves (creation-based methods only).
