...
This gives you interface-based code with normal inspector workflows and editor-time injection.
...
- `SwapProxiesWithRealInstances()` for single interface members, used at when runtime proxies are injected into the interfaces.
...
- Use interface collections (`T[]`, `List<T>`) with normal binding and injection workflows.

![Serialized interface field in the inspector](../../images/serialize-interface-inspector.webp)

If a serialized interface field is assigned an object that does not implement the expected interface, Saneject's inspector validation clears that reference to `null`.

## Runtime proxy swap hook

`SwapProxiesWithRealInstances()` is generated by the same source generator.
It is relevant when a serialized interface member currently holds a runtime proxy asset.

At runtime startup, Saneject calls this method for registered proxy swap targets.
