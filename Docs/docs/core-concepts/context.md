...
Each context also belongs to a containing serialized boundary:

- Scene objects and prefab instances placed in a scene belong to that scene.
- Prefab asset objects and nested prefab instances inside a prefab asset belong to that prefab asset.

This matters because with context isolation disabled, Saneject still does not treat every context as compatible with every other one.
It only relaxes boundaries inside the same containing scene or prefab asset.
...
- Step 1 can walk across scene object and prefab instance boundaries inside the same containing scene or prefab asset.
- Step 2 can also accept dependency candidates across those boundaries.

Values resolved through asset bindings are a separate case. They are treated as contextless for resolution purposes, so they are not filtered by scene or prefab context boundaries in the same way hierarchy-bound candidates are.
...
- `UseContextIsolation = false`: Relaxed boundaries inside the same containing scene or prefab asset. Components can be injected across contexts within that boundary, e.g., a component on a prefab instance can be injected into a component on a scene object in the same scene.
...
- The `ContextWalkFilter` dropdown in the Batch Injector window for supported scene and prefab asset runs.

## Isolation vs filtering

These features solve different problems:

- Context filtering decides what enters the run.
- Context isolation decides what gets injected where inside that run.

So you can run `AllContexts` and still keep strict boundaries by enabling context isolation.
Or you can run a narrow filter and still allow cross-context resolution by disabling context isolation.
