...
4. Matches on requested type, collection shape (single vs array/list), and optional binding qualifiers (ID, target type, member name).
...
For service-like patterns, use runtime proxies with component implementations. Runtime proxy bindings can resolve from global scope, loaded scenes, prefabs, or newly created `GameObject`s, with transient or singleton instance modes. See [Runtime proxy](runtime-proxy.md) for details.
